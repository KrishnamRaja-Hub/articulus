...
const HAND_TREES: { name: string; a: Agreement; taken: CourseId[]; valid: boolean; deferred?: string[] }[] = [
  ...
  // degenerate trees fail closed (TESTER2 M-3): never a green verdict with nothing required
  { name: 'N_OF(0): malformed, never valid', a: tree([node('N_OF', [cc('R1', [C1])], 0)], [C1]), taken: [], valid: false },
  { name: 'empty AND: malformed, never valid', a: tree([node('AND', [])]), taken: [], valid: false },
  { name: 'only optional rows: malformed, never valid', a: tree([node('AND', [cc('R1', [C1])], undefined, false)], [C1]), taken: [], valid: false },
  { name: 'an optional subtree never fails', a: tree([cc('R2', [C1]), node('AND', [cc('R1', ['1:C 9'])], undefined, false)], [C1]), taken: [C1], valid: true },
